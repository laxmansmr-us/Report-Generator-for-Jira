...
def jsonToDataFrame(params, url, auth, headers, jql, minha_empresa):
    data = []
    response = requests.get(url, headers=headers, auth=auth, params=params)
    ...
    for i in issues:
        ...
        request_atual = requests_issues(key, jql, auth, minha_empresa)
    ...
    headers = ["Key", "Time to resolution", "Time to first response", "Status Transition.date", "Status Transition.to"]
