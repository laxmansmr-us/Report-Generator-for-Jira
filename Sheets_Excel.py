...
def exportacao_xlsx(data_frames, planilhas, caminho_relatorio, tipo_relatorio):
    ...
    for i, df in enumerate(data_frames):
        # Salvar o DataFrame na planilha desejada
        df.to_excel(writer, sheet_name=planilhas[i])
        # Acessar a planilha recém-criada ou existente
...
""" 
    Dedicado a tudo que envolve Excel da parte SLA
"""
...
def criacao_tabela_semanal(slas, qntd):
    sla_firstResponse = slas[0]  # série com os valores de sla_firstResponse
    sla_resolution = slas[1]  # série com os valores de sla_resolution
    
    # Criar as semanas a partir dos índices das séries sla_firstResponse
    semanas = sla_firstResponse.index.to_series().astype(str).reset_index(drop=True)
    
    # Obter a data de início de cada semana
    start_dates = []

    for semana in semanas:
        week_number = int(semana.split()[0])
        year = date.today().isocalendar()[0]  # Get the current year
...
""" 
    Dedicado a tudo que envolve Excel da parte Status
"""
